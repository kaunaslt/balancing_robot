...
double Kp3=5, Ki3=1, Kd3=0;
PID myPID3(&Input3, &Output3, &Setpoint3, Kp3, Ki3, Kd3, DIRECT);

#define INPUT_SIZE 30

const uint32_t ENCODER_EDGES_PER_ROTATION = 682; // PPR = CPR * 4
const uint32_t ENCODER_SAMPLES_PER_SECOND = 20;              // this will need to be tuned depending on your use case...
...
int BIN1 = 24;//11; //Direction
int BIN2 = 22;//12; //Direction
...
float ACCEL_X_MIN = -271;
float ACCEL_X_MAX = 254;
float ACCEL_Y_MIN = -254;
float ACCEL_Y_MAX = 265;
float ACCEL_Z_MIN = -268;
float ACCEL_Z_MAX = 249;
...
float GYRO_AVERAGE_OFFSET_X = 30;
float GYRO_AVERAGE_OFFSET_Y = 8.4;
float GYRO_AVERAGE_OFFSET_Z = 5.1;
...
void loop()
{
  ...
  Input3 =  TO_DEG(pitch);///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  myPID3.Compute();
  move(1, Output3); //motor 1, full speed
  move(2, Output3); //motor 2, full speed
  if (myPID3.GetMode()){ if (Input3 > 30 || Input3 < -30){
  myPID3.SetMode(MANUAL);
  Output3=0;
  move(1, 0); //motor 1, full speed
  move(2, 0);
  }}
 
  if (!myPID3.GetMode()) {if (Input3 < 1 && Input3 > -1){ myPID3.SetMode(AUTOMATIC);}}
//  if (millis() > time_now + period) {
//   Serial1.print(TO_DEG(pitch)); Serial1.print(",");
//     Serial1.println(myPID3.GetMode());
//   time_now = millis();
//  }
       char input[INPUT_SIZE + 1]; //add 1 for final 0
  byte size = Serial1.readBytes(input, INPUT_SIZE);// Get next command from Serial
  ...
  int val = atoi(command);//converts the string argument str to an integer (type int).
  ++i;// increment integer
    if (i == 1){Kp3=(double)val/10;}
    if (i == 2){Ki3=(double)val/10;}
    if (i == 3){Kd3=(double)val/10; myPID3.SetTunings(Kp3, Ki3, Kd3);}
    if (i == 4){
    
    Serial1.print(Kp3);Serial1.print(",");
    Serial1.print(Ki3);Serial1.print(",");
    Serial1.println(Kd3);}
